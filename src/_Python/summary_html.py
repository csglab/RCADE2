...
def main():
    ...
    log_path = args.PREFIX + "/centrimo.log"
    ...
    my_utils.write_cisbp_meme_motif(seed_id, seed_cisbp_path, \
                                    seed_meme_path, \
                                    args.PFMS_PATH)
    ...
    cmd_centrimo = "centrimo --oc %s --score %s %s %s &>> %s" % \
                   (centrimo_out, centrimo_score, args.PEAK_PATH, \
                    opt_meme_path, log_path)
    ...
    my_utils.write_centrimo_plot(site_table, out_centrimo_graph, \
                                 centrimo_score)
    ...
    ############################################################
    my_utils.write_html(selected_motif, centrimo_graph_name, \
                        args.PREFIX, args.JOB_ID)
    ############################################################
